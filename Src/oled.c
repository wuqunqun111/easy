...
//	
//	GPIO_InitStructure.GPIO_Pin = OLED_SCLK|OLED_SDIN|OLED_RST|OLED_RS;	 //PD3,PD6推挽输出  
// 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
//	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度50MHz
// 	GPIO_Init(DATE_GPIO, &GPIO_InitStructure);	  //初始化GPIOD3,6
// 	GPIO_SetBits(DATE_GPIO,OLED_SCLK|OLED_SDIN|OLED_RST|OLED_RS);	//PD3,PD6 输出高
//	
  ...
  HAL_GPIO_Init(DATE_GPIO, &GPIO_InitStruct);
	HAL_GPIO_WritePin(DATE_GPIO,OLED_SCLK|OLED_SDIN|OLED_RST|OLED_RS,GPIO_PIN_SET);	//PD3,PD6 输出高
}
void OLED_Init(void)
{
	GPIO_Config();
